...
class TestFrontEndCommonGraphBinaryGatherer(unittest.TestCase):
    ...
    def test_call(self):
        ...
        self.assertEquals(next(iter(start_type)), ExecutableType.RUNNING)
        self.assertEquals(targets.total_processors, 3)

        test_cores = targets.get_cores_for_binary("test.aplx")
        test_2_cores = targets.get_cores_for_binary("test2.aplx")
        self.assertEquals(len(test_cores), 1)
        self.assertEquals(len(test_2_cores), 2)
