...
// The initial timeouts of the router
enum {
    ROUTER_INITIAL_DUMP_TIMEOUT = 0x4f,
    ROUTER_INITIAL_EMERGENCY_TIMEOUT = 0
};
 ...
 *
 * [The vic_control->vector_address VIC register] must be read at the start of
 * the ISR, and written at the end of the ISR to signal that the priority
 * logic should update to the next priority interrupt. */

static inline void vic_signal_interrupt_start(void) {
    (void) vic_control->vector_address;
}

static inline void vic_signal_interrupt_done(void) {
...
static inline bool router_unblocked(void) {
    uint stage = router->status.output_stage;
    return stage == output_stage_empty || stage == output_stage_full;
}
...
static INT_HANDLER reinjection_ready_to_send_callback(void) {
    ...
    if (router_unblocked()) {
        ...
        if (pkt_queue.tail != pkt_queue.head) {
            // dequeue packet,
            router_packet_header_t hdr = pkt_queue.queue[pkt_queue.head].hdr;
...
static INT_HANDLER reinjection_dropped_packet_callback(void) {
    ...
    router_packet_header_t hdr = router->dump.header;
    uint pld = router->dump.payload;
    uint key = router->dump.key;

    // clear dump status and interrupt in router
    // NB: read status LAST
    router_dump_outputs_t rtr_dump_outputs = router->dump.outputs;
    router_dump_status_t rtr_dstat = router->dump.status;
    uint is_processor_dump = rtr_dump_outputs.processor;
    uint is_link_dump = rtr_dump_outputs.link;
...
static inline void reinjection_set_timeout(uint payload) {
    router->control.drop_wait_time = payload;
}

static inline void reinjection_set_emergency_timeout(uint payload) {
    router->control.emergency_wait_time = payload;
}
...
static inline int reinjection_set_timeout_sdp(sdp_msg_t *msg) {
    ...
    router->control.drop_wait_time = msg->arg1;
...
static inline int reinjection_get_status(sdp_msg_t *msg) {
    ...
    router_control_t control = router->control;
    data->router_timeout = control.drop_wait_time;
    data->router_emergency_timeout = control.emergency_wait_time;
...
static void reinjection_configure_timer(void) {
    // Clear the interrupt
    timer1->control.interrupt_enable = 0;
    timer1->interrupt_clear = 1;

    // Set the timer times
    timer1->load_value = sv->cpu_clk * TICK_PERIOD;
    timer1->background_load_value = sv->cpu_clk * TICK_PERIOD;
}
...
static void reinjection_configure_router(void) {
    ...
    router_control_t control = router->control;
    control.drop_wait_time = ROUTER_INITIAL_DUMP_TIMEOUT;
    control.emergency_wait_time = ROUTER_INITIAL_EMERGENCY_TIMEOUT;
    ...
    (void) router->dump.status;

    // clear router error status,
    (void) router->error.status;

    // and enable router interrupts when dumping packets, and count errors
    control.dump_interrupt_enable = 1;
    control.count_framing_errors = 1;
    control.count_parity_errors = 1;
    control.count_timestamp_errors = 1;
    router->control = control;
}
...
static inline void data_out_dma_read(
    ...
    dma_controller->tcm_address = destination;
    dma_controller->description = desc;
}
...
static INT_HANDLER data_out_dma_complete(void) {
    ...
    // reset the interrupt.
    dma_controller->control.clear_done_int = 1;
    if (!stop) {
...
//! \brief the handler for DMA errors (hopefully unlikely...)
static INT_HANDLER data_out_dma_error(void) {
    vic_signal_interrupt_start();
    io_printf(IO_BUF, "DMA failed: 0x%08x\n", dma_controller->status);
    dma_controller->control.restart = 1; // TODO: This makes no sense
    vic_signal_interrupt_done();
    rt_error(RTE_DABT);
}
...
static INT_HANDLER data_out_dma_timeout(void) {
    ...
    dma_controller->control.clear_timeout_int = 1;
    vic_signal_interrupt_done();
}
...
static void reinjection_initialise(void) {
    ...
    vic_set_callback(CC_SLOT, CC_TNF_INT, reinjection_ready_to_send_callback);

    // Setup the timer interrupt
    vic_set_callback(TIMER_SLOT, TIMER1_INT, reinjection_timer_callback);
...
static void data_out_initialise(void) {
    ...
    vic_set_callback(DMA_SLOT, DMA_DONE_INT, data_out_dma_complete);
    vic_set_callback(DMA_ERROR_SLOT, DMA_ERR_INT, data_out_dma_error);
    vic_set_callback(DMA_TIMEOUT_SLOT, DMA_TO_INT, data_out_dma_timeout);
