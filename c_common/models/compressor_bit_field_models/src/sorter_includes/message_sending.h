...
static inline bool store_sdram_addresses_for_compression(
        int n_rt_addresses, uint32_t core_index, uint32_t mid_point,
        comp_core_store_t* cores_bf_tables,
        table_t** bit_field_routing_tables){

    //free previous if there is any

    if (cores_bf_tables[core_index].elements != NULL) {
        ...
        FREE(cores_bf_tables[core_index].elements);
    }

    // store the elements into the tracker.
    cores_bf_tables[core_index].n_elements = n_rt_addresses;
    cores_bf_tables[core_index].n_bit_fields = mid_point;
    cores_bf_tables[core_index].elements = bit_field_routing_tables;
    return true;
}
...
static inline void update_mc_message(
        int core_index, sdp_msg_pure_data* my_msg) {
    ...
    my_msg->dest_port =
        (RANDOM_PORT << PORT_SHIFT) | core_index;
}
...
//! \param[in] my_msg: sdp message to send
//! \param[in] core_index: the processor to use
//! available right now.
...
static bool message_sending_set_off_bit_field_compression(
    ...
    log_info(
        ...
        core_index, n_rt_addresses, n_entries, mid_point);

    // allocate space for the compressed routing entries if required
    if (cores_bf_tables[core_index].compressed_table == NULL){
        cores_bf_tables[core_index].compressed_table = MALLOC_SDRAM(
    ...
    bool success = store_sdram_addresses_for_compression(
        n_rt_addresses, core_index, mid_point, cores_bf_tables,
    ...
    set_up_packet(&cores_bf_tables[core_index], my_msg);

    // update sdp to right destination
    update_mc_message(core_index, my_msg);
...
//! \param[in] core_index: The processor id to use
...
bool message_sending_set_off_no_bit_field_compression(
        comp_core_store_t* cores_bf_tables, sdp_msg_pure_data* my_msg,
    ...
    return message_sending_set_off_bit_field_compression(
        N_UNCOMPRESSED_TABLE, 0, cores_bf_tables,
        bit_field_routing_tables, my_msg, core_index);
}
