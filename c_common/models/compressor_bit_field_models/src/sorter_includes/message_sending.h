...
//! \param[in] processor_bf_tables: the map of compressor processor and the
//! SDRAM addresses for its routing tables to compress.
...
static inline bool store_sdram_addresses_for_compression(
    ...
    if (processor_bf_tables[processor_id].elements != NULL) {
        ...
        if (!success) {
            log_error("failed to free compressor processor elements.");
            return false;
        }
    ...
    processor_bf_tables[processor_id].n_elements = n_rt_addresses;
    processor_bf_tables[processor_id].n_bit_fields = mid_point;
    processor_bf_tables[processor_id].elements = bit_field_routing_tables;
    return true;
}
...
//! \brief sends a SDP message to a compressor processor to do compression with
...
//! \param[in] processors_bf_tables: struct that holds meta data like the
//! routing table addresses for each compressor processor attempt.
//! \param[in] bit_field_routing_tables: the list of routing tables which
//! were generated by the set of bitfields.
//! \param[in] my_msg: sdp message to send
//! \param[in] processor_id: the processor to use
//! available right now.
...
static bool message_sending_set_off_bit_field_compression(
        ...
        comp_processor_store_t* processors_bf_tables,
    ...
    if (processors_bf_tables[processor_id].compressed_table == NULL){
        processors_bf_tables[processor_id].compressed_table = MALLOC_SDRAM(
    ...
    bool success = store_sdram_addresses_for_compression(
        n_rt_addresses, processor_id, mid_point, processors_bf_tables,
    ...
    // generate the message to send to the compressor processor
    set_up_packet(&processors_bf_tables[processor_id], my_msg);

    // update sdp to right destination
    update_mc_message(processor_id, my_msg);
...
bool message_sending_set_off_no_bit_field_compression(
    ...
    return message_sending_set_off_bit_field_compression(
        N_UNCOMPRESSED_TABLE, 0, processors_bf_tables,
