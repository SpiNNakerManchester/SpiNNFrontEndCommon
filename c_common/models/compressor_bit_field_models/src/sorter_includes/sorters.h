    ...
    do {
        ...
        for (uint index = 1; index < length_of_internal_array; index ++) {
            ...
            if (element < compare_element) {
                ...
                proc_cov_by_bit_field[worst_core_id]->redundant_packets[index - 1] =
                        compare_element;

                // move element over to compare location
                proc_cov_by_bit_field[worst_core_id]->redundant_packets[index] =
                        temp_value;

                // update flag
                moved = true;
            } else {  // jump to next element
                element = proc_cov_by_bit_field[worst_core_id]->redundant_packets[index];
            }
        }
    } while (moved);
}
    ...
    do {
        ...
        for (uint index = 1; index < length_of_array; index++) {
            ...
            if (element->n_redundant_packets <
                ...
                // move to temp
                coverage_t *temp_pointer = element;
                // move compare over to element
