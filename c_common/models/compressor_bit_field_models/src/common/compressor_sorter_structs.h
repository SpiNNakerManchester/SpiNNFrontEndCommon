...
typedef enum compressor_states {
   ...
   FORCED_BY_COMPRESSOR_CONTROL = 34, SUCCESSFUL_COMPRESSION = 35,
   FAILED_TO_COMPRESS = 36, RAN_OUT_OF_TIME = 37
} compressor_states;

//! \brief messages from the sorter to the compressor.
typedef enum instructions_to_compressor {
    NOTHING = 40, PREPARE = 41,  RUN = 42, FORCE_TO_STOP = 44
...
//! \brief the data stuct being sent from the sorter to the compressor
