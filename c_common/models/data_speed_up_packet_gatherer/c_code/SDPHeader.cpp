...
SDPHeader::SDPHeader(
        ...
        int destination_port, int flags, int tag, int source_port,
        int source_cpu, int source_chip_x, int source_chip_y){
...
char * SDPHeader::convert_byte_array(){
    ...
    tmp = (
        (this->destination_port & 7) << 5) | (this->destination_chip_p & 31);
    ...
    memcpy(message_data+6*sizeof(uint8_t), &this->destination_chip_y,
           sizeof(uint8_t));
    memcpy(message_data+7*sizeof(uint8_t), &this->destination_chip_x,
           sizeof(uint8_t));
    memcpy(message_data+8*sizeof(uint8_t), &this->source_chip_y,
           sizeof(uint8_t));
    memcpy(message_data+9*sizeof(uint8_t), &this->source_chip_x,
           sizeof(uint8_t));
