...
class DatabaseWriter(object):
    ...
    def add_system_params(
            self, time_scale_factor, default_machine_time_step, runtime):
        """ Write system params into the database

        :param time_scale_factor: the time scale factor used in timing
        :param default_machine_time_step: the machine time step used in timing
        :param runtime: the amount of time the application is to run for
        """
        with self._connection:
            self.__insert_cfg("machine_time_step", default_machine_time_step)
