...
class ReadStatusProcess(AbstractMultiConnectionProcess):
    def __init__(self, connection_selector):
        ...
        self._re_injection_status = dict()

    def handle_re_injection_status_response(self, response):
        status = response.reinjection_functionality_status
        self._re_injection_status[(response.sdp_header.source_chip_x,
                                   response.sdp_header.source_chip_y)] = status

    def get_re_injection_status(self, x, y, p, command_code):
        self._re_injection_status = dict()
        self._send_request(GetReinjectionStatusMessage(x, y, p, command_code),
                           callback=self.handle_re_injection_status_response)
        self._finish()
        self.check_for_error()
        return self._re_injection_status[(x, y)]
    ...
    def get_reinjection_status_for_core_subsets(
        ...
        self._finish()
        self.check_for_error()
        return self._re_injection_status
