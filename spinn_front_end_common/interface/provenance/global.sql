...
CREATE VIEW IF NOT EXISTS full_timer_view AS
    SELECT timer_id, category, algorithm, work, n_machine, timer_provenance.time_taken, n_reset, n_run, n_loop, skip_reason
...
CREATE VIEW IF NOT EXISTS timer_view AS
    SELECT category, algorithm, work, n_machine, time_taken, n_reset, n_run, n_loop
...
CREATE TABLE IF NOT EXISTS category_timer_provenance(
    ...
    time_taken INTEGER,
    n_machine INTEGER NOT NULL,
    n_run INTEGER NOT NULL,
    n_loop INTEGER);

CREATE VIEW IF NOT EXISTS category_timer_view as
    SELECT category_id, category, time_taken, n_machine, n_reset, n_run, n_loop
    FROM
