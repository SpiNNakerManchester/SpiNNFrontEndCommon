...
class RouterProvenanceGatherer(object):
    ...
    def _write_router_table_diagnostic(
        ...
        if not machine.get_chip_at(x, y).virtual:
            try:
                ...
                seen_chips.add((x, y))
                re_injection_status = re_injection_data[(x, y)]
                items.extend(self._write_router_diagnostics(
                    x, y, router_diagnostic, re_injection_status, True,
                    router_table))
                self._add_totals(router_diagnostic, re_injection_status)
        ...
        if not chip.virtual and (chip.x, chip.y) not in seen_chips:
            try:
                ...
                if (diagnostic.n_dropped_multicast_packets or
                    ...
                    re_injection_status = re_injection_data[(chip.x, chip.y)]
                    items.extend(self._write_router_diagnostics(
                            chip.x, chip.y, diagnostic, re_injection_status,
                            False, None))
                    self._add_totals(diagnostic, re_injection_status)
            except Exception:
                # There could be issues with unused chips - don't worry!
                pass

    def _add_totals(self, router_diagnostic, re_injection_status):
        self._total_sent_packets += (
        ...
        if re_injection_status is not None:
            self._total_missed_dropped_packets += (
                re_injection_status.n_missed_dropped_packets)
            self._total_lost_dropped_packets += (
    ...
    def _write_router_diagnostics(
            self, x, y, router_diagnostic, re_injection_status, expected,
            router_table):
        ...
        :param re_injection_status: the data gained from the extra monitor \
        ...
        if re_injection_status is not None:
            items.append(ProvenanceDataItem(
                ...
                re_injection_status.n_dropped_packets))
            items.append(ProvenanceDataItem(
                self._add_name(names, "Missed_For_Reinjection"),
                re_injection_status.n_missed_dropped_packets,
                report=re_injection_status.n_missed_dropped_packets > 0,
                message=(
                    "The extra monitor on {}, {} has missed {} "
                    "packets.".format(
                        x, y, re_injection_status.n_missed_dropped_packets))))
            items.append(ProvenanceDataItem(
                self._add_name(names, "Reinjection_Overflows"),
                re_injection_status.n_dropped_packet_overflows,
                report=re_injection_status.n_dropped_packet_overflows > 0,
                message=(
            ...
            items.append(ProvenanceDataItem(
                ...
                str(re_injection_status.n_link_dumps),
                report=re_injection_status.n_link_dumps > 0,
                message=(
                    ...
                    .format(x, y, re_injection_status.n_link_dumps))))
            items.append(ProvenanceDataItem(
                self._add_name(names, "Dumped_from_a_processor"),
                str(re_injection_status.n_processor_dumps),
                report=re_injection_status.n_processor_dumps > 0,
                message=(
                    "The extra monitor on {}, {} has detected that {} packets "
                    ...
                    " number is likely a overestimate.".format(
                        x, y, re_injection_status.n_processor_dumps))))
        return items
