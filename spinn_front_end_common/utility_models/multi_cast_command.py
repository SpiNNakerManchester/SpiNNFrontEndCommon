...
def get_time_before_end(time):
    """ Get a time for a command that is a number of timesteps before the\
        end of the simulation

    :param time: The time before the end of the simulation to get the time for
    """
    return -time - 1
...
class MultiCastCommand(object):
    ...
    def __init__(self, time, key, mask=0xFFFFFFFF, payload=None, repeat=0,
        ...
        if repeat < 0 or repeat > 0xFFFF:
            raise SpinnFrontEndException("repeat must be between 0 and 65535")
    ...
    def get_payload(self, routing_info, graph):
        ...
        :param graph: The partitioned graph for which the routing\
